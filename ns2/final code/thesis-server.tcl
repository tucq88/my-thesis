...
Thesis instproc init {} {
    ...
    set out "OUT-$tcptyp-$tcpvar-server"
    set f2 [open $out w]
    $ns_ trace-all $f2
}
...
#Lien ket cac node
	$ns_ duplex-link $nds $nsnoop 10Mb 2ms DropTail
	$ns_ queue-limit $nds $nsnoop $opt(qsize)
	
	$ns_ duplex-link $nsg1 $nsc 10Mb 2ms DropTail
	$ns_ queue-limit $nsg1 $nsc $opt(qsize)
...
#Tao loi ngau nhien tren ket noi ve tinh
	...
	$em1_ ranvar $exprv1
	#$em1_ ranvar [new RandomVariable/Exponential]
	...
	$em2_ ranvar $exprv2
	#$em2_ ranvar [new RandomVariable/Exponential]
...
# Ket noi TCP client va TCP server
	...
	$ns_ at 1.0 "$ftp1 start"
        ...
        puts "Dung o thoi diem 2000s"
        $ns_ at 2000.0 "$self finish"
