...
$self instvar ns_ nsc nsg1 nsgeo nsg2 nds nsnoop tcp1 f0 nf tcpvar tcptyp
puts "Begin of Test!"
...
lappend nodelist $nsg1
    set lan [$ns_ make-lan $nodelist $opt(bw) \ $opt(delay) $opt(11) $opt(ifq) $opt(mac) $opt(chan)]
    if {$tcptyp == "Snoop"} {
	set opt(11) LL/LLSnoop
	set opt(ifq) $opt(ifq)
	$opt(ifq) set limit_ 1000
    }
    $lan addNote [list $nsnoop] $opt(bw) $opt(delay) $opt(11) $opt(ifq) $opt(mac)
#Linking nodes
